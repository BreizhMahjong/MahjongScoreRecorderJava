...
public class UITabPanelManagePlayer extends UITabPanel {
	...
	public UITabPanelManagePlayer(final DataAccessManagePlayer dataAccess) {
			...
			0);
		{
				...
				0);
			{
				...
				final ProportionalGridLayout layout = new ProportionalGridLayout(
					1,
					7,
					2,
					2);
				addPlayerPanel.setLayout(
					layout);
				...
				addPlayerC.gridWidth = 2;
				addPlayerPanel.add(
					textNewPlayerDisplayName,
					addPlayerC);

				buttonAddPlayer = new JButton(
					"Ajouter");
				buttonAddPlayer.setPreferredSize(
					buttonMinSize);
				addPlayerC.x = 6;
				addPlayerC.gridWidth = 1;
				addPlayerPanel.add(
...

			{
				...
				modifyPlayerC.x = 6;
				modifyPlayerC.gridWidth = 1;
				buttonDeletePlayer = new JButton(
					"Supprimer");
				buttonDeletePlayer.setPreferredSize(
					buttonMinSize);
				modifyPlayerPanel.add(
					buttonDeletePlayer,
					modifyPlayerC);
				...
				modifyPlayerC.x = 1;
				modifyPlayerC.gridWidth = 2;
				checkBoxFrequent = new JCheckBox(
					"Fréquent");
				modifyPlayerPanel.add(
					checkBoxFrequent,
					modifyPlayerC);
				...
				modifyPlayerC.x = 6;
				modifyPlayerC.gridWidth = 1;
				buttonModifyPlayer = new JButton(
					"Modifier");
				buttonModifyPlayer.setPreferredSize(
					buttonMinSize);
				modifyPlayerPanel.add(
					buttonModifyPlayer,
					modifyPlayerC);
		...
		comboBoxPlayer.addActionListener(
			comboBoxPlayerActionListener);
		buttonAddPlayer.addActionListener(
			(final ActionEvent e) -> addPlayer());
		buttonModifyPlayer.addActionListener(
			(final ActionEvent e) -> modifyPlayer());
		buttonDeletePlayer.addActionListener(
			(final ActionEvent e) -> deletePlayer());
	}
	...
	private void refreshPlayer() {
		new Thread(
			() -> {
				try {
					comboBoxPlayer.removeActionListener(
						comboBoxPlayerActionListener);
					...
					for (int playerIndex = 0; playerIndex < listPlayer.size(); playerIndex++) {
						final Player player = listPlayer.get(
							playerIndex);
						final String displayString = Integer.toString(
							player.getPlayerID()) + " - " + player.getPlayerName() + " - " + player.getDisplayName();
						comboBoxPlayer.addItem(
							displayString);
					}
					...
					if (listPlayer.size() > 0) {
						comboBoxPlayer.setSelectedIndex(
							0);
					} else {
						comboBoxPlayer.setSelectedIndex(
							-1);
	...
	private void addPlayer() {
		...
		if (playerName != null && playerName.length() > 0 && playerDisplayName != null && playerDisplayName.length() > 0) {
			final UpdateResult result = dataAccess.addPlayer(
				playerName,
				playerDisplayName);
			if (result.getResult()) {
				JOptionPane.showMessageDialog(
	...
	private void displayPlayer() {
		...
		if (selectedPlayerIndex != -1) {
			...
			checkBoxFrequent.setSelected(
				player.isFrequent());
			checkBoxRegular.setSelected(
				player.isRegular());
	...
	private void modifyPlayer() {
		...
		if (selectedPlayerIndex != -1) {
			...
			if (playerName != null && playerName.length() > 0 && playerDisplayName != null && playerDisplayName.length() > 0) {
				final UpdateResult result = dataAccess.modifyPlayer(
					player.getPlayerID(),
					playerName,
					playerDisplayName,
					frequent,
	...
	private void deletePlayer() {
		...
		if (selectedPlayerIndex != -1) {
			final Player player = listPlayer.get(
				selectedPlayerIndex);
			final UpdateResult result = dataAccess.deletePlayer(
				player.getPlayerID());
			if (result.getResult()) {
				JOptionPane.showMessageDialog(
					this,
						...
						try {
							...
							for (int index = 0; index < listPlayer.size(); index++) {
								...
								writer.write(
									SEPARATOR);
								writer.write(
									player.getLicense() != null ? player.getLicense() : "");
							}
						} catch (final Exception e) {
							JOptionPane.showMessageDialog(
								this,
