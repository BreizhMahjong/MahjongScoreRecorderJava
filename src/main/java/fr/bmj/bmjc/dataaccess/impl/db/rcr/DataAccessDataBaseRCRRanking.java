...
public class DataAccessDataBaseRCRRanking extends DataAccessDataBaseRCRCommon implements DataAccessRCRRanking {
	...
	private float getProportionalPeriod(final long from,
		final long to) {
		if (from < to) {
		...
		switch (periodMode) {
			...
			case YEAR:
				...
				calendarTo.add(Calendar.YEAR,
					1);
				minimumGames = Math.round(getProportionalPeriod(calendarFrom.getTimeInMillis(),
		...
		try {
			switch (rankingMode) {
				case TOTAL_FINAL_SCORE: {
					...
					final String queryRegularPart = onlyRegularPlayers
						? " AND player.regular=TRUE"
						: "";
					...
					final String queryHavingPart = useMinimumGame
						? " HAVING COUNT(*)>="
							+ Integer.toString(minimumGames)
						: "";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY total DESC"
						: " ORDER BY total ASC";
					PreparedStatement statement = null;
					if (periodMode == EnumPeriodMode.ALL) {
						statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryGroupPart + queryHavingPart + queryOrderPart);
						statement.setShort(1,
							tournament.getId());
					} else {
						statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryPeriodPart + queryGroupPart + queryHavingPart + queryOrderPart);
						statement.setShort(1,
							tournament.getId());
						statement.setDate(2,
					...
					final ResultSet result = statement.executeQuery();
					while (result.next()) {
						final RCRTotalScore total = new RCRTotalScore(result.getString(1),
							result.getString(2),
							0,
							0,
							0);
				...
				case BEST_FINAL_SCORE: {
					...
					final String queryPeriodPart = " AND rcr_game_id.date>=? AND rcr_game_id.date<?";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY rcr_game_score.final_score DESC"
						: " ORDER BY rcr_game_score.final_score ASC";
					final String queryFetchPart = " FETCH FIRST "
						+ Integer.toString(NUMBER_TOP)
						+ " ROWS ONLY";
					PreparedStatement statement = null;
					if (periodMode == EnumPeriodMode.ALL) {
						...
						statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryPeriodPart + queryOrderPart + queryFetchPart);
						statement.setShort(1,
							tournament.getId());
						statement.setDate(2,
							new Date(calendarFrom.getTimeInMillis()));
				...
				case BEST_GAME_SCORE: {
					...
					final String queryPeriodPart = " AND rcr_game_id.date>=? AND rcr_game_id.date<?";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY rcr_game_score.game_score DESC"
				...
				case WIN_RATE_4: {
					final Map<String, RCRTotalScore> mapNameScore = new HashMap<String, RCRTotalScore>();
					{
						...
						PreparedStatement statement = null;
						if (periodMode == EnumPeriodMode.ALL) {
							statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryGroupPart + queryHavingPart);
							statement.setShort(1,
								tournament.getId());
						} else {
							statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryPeriodPart + queryGroupPart + queryHavingPart);
							statement.setShort(1,
								tournament.getId());
							statement.setDate(2,
								new Date(calendarFrom.getTimeInMillis()));
						...
						while (result.next()) {
							...
							mapNameScore.put(total.displayName,
								total);
						}
						result.close();
						statement.close();
					}
					{
						...
						if (periodMode == EnumPeriodMode.ALL) {
							...
							statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryPeriodPart + queryGroupPart);
							statement.setShort(1,
								tournament.getId());
							statement.setDate(2,
								new Date(calendarFrom.getTimeInMillis()));
					...
					switch (sortingMode) {
						case DESCENDING:
							Collections.sort(rankingScores,
								(final RCRTotalScore o1,
									final RCRTotalScore o2) -> -Double.compare(o1.totalScore.doubleValue(),
				...
				case WIN_RATE_5: {
					final Map<String, RCRTotalScore> mapNameScore = new HashMap<String, RCRTotalScore>();
					{
						...
						final String queryHavingPart = useMinimumGame
							? " HAVING COUNT(*)>="
								+ Integer.toString(minimumGames / 4)
							: "";
						PreparedStatement statement = null;
						if (periodMode == EnumPeriodMode.ALL) {
							statement = dataBaseConnection.prepareStatement(querySelectPart + queryWherePart + queryRegularPart + queryGroupPart + queryHavingPart);
				...
				case ANNUAL_TOTAL_FINAL_SCORE: {
					...
					final String querySubRegularPart = onlyRegularPlayers
						? " AND player.regular=TRUE"
						: "";
					...
					final String queryHavingPart = useMinimumGame
						? " HAVING COUNT(*)>="
							+ Integer.toString(MINIMUM_GAME_YEAR)
						: "";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY total DESC"
						: " ORDER BY total ASC";
					final String queryFetchPart = " FETCH FIRST "
						+ Integer.toString(NUMBER_TOP)
						+ " ROWS ONLY";
					final PreparedStatement statement = dataBaseConnection.prepareStatement(querySelectPart + querySubSelectPart + querySubWherePart + querySubRegularPart + querySubFinalPart + queryGroupPart + queryHavingPart + queryOrderPart + queryFetchPart);
					statement.setShort(1,
						tournament.getId());
				...
				case TRIMESTRIAL_TOTAL_FINAL_SCORE: {
					...
					final String queryHavingPart = useMinimumGame
						? " HAVING COUNT(*)>="
							+ Integer.toString(MINIMUM_GAME_TRIMESTER)
						: "";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY total DESC"
						: " ORDER BY total ASC";
				...
				case MENSUAL_TOTAL_FINAL_SCORE: {
					...
					final String queryHavingPart = useMinimumGame
						? " HAVING COUNT(*)>="
							+ Integer.toString(MINIMUM_GAME_MONTH)
						: "";
					final String queryOrderPart = sortingMode == EnumSortingMode.DESCENDING
						? " ORDER BY total DESC"
						: " ORDER BY total ASC";
