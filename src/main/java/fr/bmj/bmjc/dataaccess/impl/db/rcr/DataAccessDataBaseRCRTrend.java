		...
		try {
...

			{
				...
				while (result.next()) {
					...
					scoreListPlayerNames.set(dateIndex,
						scoreListPlayerNames.get(dateIndex) + score);
					final List<Integer> scoreListDisplayNames = dataWithDisplayName.get(result.getString(3));
					scoreListDisplayNames.set(dateIndex,
						scoreListDisplayNames.get(dateIndex) + score);
				}
			}
			return new RCRDataPackageTrend(dates,
				dataWithPlayerName,
				dataWithDisplayName);
